...
This document describes the technical architecture of the Remootio custom component for Home Assistant.
...
│   ├── __init__.py          # Exports RemootioDataUpdateCoordinator
...
│   ├── __init__.py          # Exports RemootioEntity
...
- `base.py` - Main coordinator class (`RemootioDataUpdateCoordinator`)
...
**Key class:** `RemootioDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `RemootioConfigFlowHandler` (main flow)
- `RemootioOptionsFlow` (options)
...
**Key class:** `RemootioEntity` (in `entity/base.py`)
...
2. `RemootioEntity` for common functionality
...
1. Create directory: `custom_components/remootio/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `RemootioEntity`
